...
public final class SLBlockNode extends SLStatementNode implements BlockNode.ElementExecutor<SLStatementNode> {
    ...
    protected boolean hasNewChildNode() {
        ...
        for (SLStatementNode child : block.getElements()) {
    ...
    public void handleAsReplaced(int i) {
        ...
        for (int j = 0; j < elements.length; j++) {
            ...
            if (i < sum + size) {
                ...
                delNode.setIdentifier(NodeIdentifier.create(ident.getFunctionNumber(), 1, true));
                delNode.setNewNode();
                list.add(j, delNode);
                block = BlockNode.create(list.toArray(new SLStatementNode[0]), this);
                element.setIdentifier(NodeIdentifier.create(ident.getFunctionNumber(), 0, true));
                element.setNewNode();
                return;
            }
            sum += size + 1;
