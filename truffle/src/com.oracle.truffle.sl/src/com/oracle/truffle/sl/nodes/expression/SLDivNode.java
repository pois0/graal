...
public abstract class SLDivNode extends SLBinaryNode {
    ...
    public Object calcGeneric(VirtualFrame frame) {
        ...
        try {
            if (leftInterop.fitsInLong(left) && rightInterop.fitsInLong(right)) {
                return div(leftInterop.asLong(left), rightInterop.asLong(right));
            } else if (left instanceof SLBigNumber && right instanceof SLBigNumber) {
                return div((SLBigNumber) left, (SLBigNumber) right);
            } else {
                return 0;
            }
    ...
    public boolean isEqualNode(SLStatementNode that) {
        if (!(that instanceof SLDivNode)) return false;
        SLDivNode thatAdd = (SLDivNode) that;
